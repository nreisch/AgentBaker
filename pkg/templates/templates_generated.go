...
function Install-GmsaPlugin {
    ...
    # Retry 100 times until it succeeds
    Write-Log "Enabling the PowerShell privilege"
    $enablePrivilege = $false
    for ($i = 0; $i -lt 100; $i++) {
        $enablePrivilege = Enable-Privilege SeTakeOwnershipPrivilege
        if ($enablePrivilege) {
            Write-Log "Enable-Privilege succeeded"
            break
        }
        Start-Sleep 1
    }
...
$global:WINDOWS_CSE_ERROR_GMSA_ENABLE_POWERSHELL_PRIVILEGE=33
$global:WINDOWS_CSE_ERROR_GMSA_SET_REGISTRY_PERMISSION=34`)
